...
export const registerUser = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { email, password, acceptPromotions, phone, name, provider } =
      req.body;
    ...
    try {
      ...
      if (findIntegration && !password) {
        // add userHash to the integration
...
export const loginUser = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    if (!user.hashedPassword)
      return next(
        new ErrorResponse({
          message: "Please login with exernal provider",
          statusCode: 401,
        })
      );
...
export const getCurrentUser = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    if (req.user) {
      const response = excludeFields(req.user, [
        "hashedPassword",
        "hashedResetToken",
        "timestamp",
      ]);
...
export const updateUserDetails = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const { email, name, phone, asWhatsapp, acceptPromotions, pfp, hash } =
      req.body;
      ...
      try {
        ...
        if (!user) {
          ...
          if (!isMatch) {
            ...
            const response = excludeFields(updatedUser, [
              "hashedPassword",
              "hashedResetToken",
              "timestamp",
            ]);
...
export const getUserByHash = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
