...
const Appointments = () => {
  ...
  const handleCancelAppointment = async (appointment: Appointment) => {
    ...
    if (cancelAppointment.success) {
      ...
      const deletedFromGoogle = await deleteData(
        `/googleAppointments/${user.email}?googleEventId=${appointment.googleEventId}&googleWriteInto=${calendar.googleWriteInto}`
      );
      if (deletedFromGoogle.success) {
        ...
        setAlert({
          message: t(
            "There was a problem canceling the appointment on your google calendar, please do it manually."
          ),
          severity: "warning",
          code: 0,
        });
    ...
    <div style={{ display: "flex", flexDirection: "column" }}>
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr" }}>
        <FormControl>
          ...
          <Select
            value={sorting}
            onChange={(e) => handleSorting(e.target.value as string)}
          >
        ...
        <FormControl>
          ...
          <Select
            ...
            onChange={(e) => handleStatusFilter(e.target.value as string)}
          >
              ...
              >
                ...
                {appointment.status === "canceled" ? (
                  ...
                  <TableCell>
                    ...
                    <Button
                      onClick={() =>
                        handleRescheduleAppointment(appointment.hash)
                      }
                    >
                      {t("Reschedule")}
                    </Button>
                    {appointment.status !== "confirmed" && (
                      <Button
                        onClick={() =>
                          handleApproveAppointment(appointment.hash)
                        }
                      >
                        {t("Approve")}
                      </Button>
