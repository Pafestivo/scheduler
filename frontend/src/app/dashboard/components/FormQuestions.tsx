...
const reducer = (state: State, action: FormQuestionAction): State => {
  ...
  switch (action.type) {
    ...
    case "UPDATE_TYPE":
      ...
      if (
        action.value === "select" &&
        !updatedQuestions[action.index].options?.length
      ) {
        updatedQuestions[action.index].options = [
          { id: generateHash(Math.random().toString()), value: "" },
        ];
      }
      return updatedQuestions;
    ...
    case "UPDATE_OPTIONS": {
      ...
      action.value.forEach((newOption) => {
        const existingOption = questionToUpdate.options.find(
          (option: OptionObject) => option.id === newOption.id
        );
        if (existingOption) {
    ...
    case "ADD_OPTION":
      updatedQuestions[action.index].options.push({
        id: generateHash(Math.random().toString()),
        value: "",
      });
      return updatedQuestions;
    case "DELETE_QUESTION":
      ...
      return updatedQuestions;
    case "DELETE_OPTION":
      updatedQuestions[action.index].options = updatedQuestions[
        action.index
      ].options.filter((option: OptionObject) => option.id !== action.id);
      return updatedQuestions;
    default:
      throw new Error();
  }
  ...
  const updatePersonalForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (calendar) {
        ...
        setAlert({
          code: 200,
          message: t("Personal form updated!"),
          severity: "success",
        });
        setAlertOpen(true);
      }
  ...
  return (
    <Box component="form" onSubmit={updatePersonalForm}>
      <p style={{ fontSize: "0.8rem" }}>
        {t(
          "*Note that we ask for the name, phone, and email! Adding these questions yourself will result in duplication."
        )}
      </p>
