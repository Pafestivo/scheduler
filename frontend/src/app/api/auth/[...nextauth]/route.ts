...
const AuthorizationParams: IAuthorizationOptions = {
  params: {
    ...
    access_type: "offline",
    response_type: "code",
    scope: "https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/admin.directory.resource.calendar"
  }
}
...
const handler = NextAuth({
  providers: [
    GoogleProvider(authOptions.GoogleProvider),
  ],
  session: {
    strategy: 'jwt',
  },
  callbacks: {
    async signIn(user) {
      ...
      if(user.account) {
        ...
        await postData('/integration', {token: access_token, refreshToken: refresh_token, expiresAt: expires_at, userEmail: email, provider})
      }    
      return true
    },
